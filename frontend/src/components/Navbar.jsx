    ...
    <header className="sticky top-0 z-40 border-b border-white/10 bg-brand-navy/95 backdrop-blur-md">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 flex items-center justify-between h-16">
        <Link to="/" className="flex items-center gap-2 group" onClick={() => setOpen(false)}>
          <span className="h-9 w-9 rounded-lg bg-white/95 shadow-md grid place-items-center overflow-hidden">
            <img
              src={umLogo}
        ...
        <nav className="hidden md:flex items-center gap-1">
          ...
          >
            Cart
            {itemCount > 0 ? (
              <span className="absolute -top-0.5 -right-0.5 min-w-[1.125rem] h-[1.125rem] rounded-full bg-brand-sky text-[10px] font-bold text-white grid place-items-center px-1">
          ...
          <button
            type="button"
            onClick={openQuote}
            className="ml-2 rounded-lg bg-brand-orange px-4 py-2 text-sm font-semibold text-white hover:bg-brand-sky transition shadow-md"
          >
            Free quote
          </button>
        </nav>
        ...
        <div className="flex md:hidden items-center gap-2">
          <button
            type="button"
            onClick={openQuote}
            className="rounded-lg bg-brand-orange px-3 py-2 text-xs font-semibold text-white hover:bg-brand-sky transition"
          >
            Quote
          </button>
          <button
      ...
      {open && (
        <div className="md:hidden border-t border-white/10 bg-brand-navy px-4 pb-4 space-y-1">
          ...
          >
            <span>Cart</span>
            {itemCount > 0 ? (
              <span className="rounded-full bg-brand-sky text-white text-xs font-bold px-2 py-0.5">
                {itemCount}
              </span>
            ) : null}
          </Link>
